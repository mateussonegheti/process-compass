-- Remover liberação automática de processos ao capturar novo
-- Manter processo com avaliador até ação explícita (Salvar ou Finalizar)
...
DECLARE
  ...
  v_pecas_tipos text;
  v_pecas_ids text;
  v_tem_processo_em_analise boolean;
BEGIN
  -- 1. Verificar se usuário já tem processo em análise neste lote
  SELECT EXISTS(
    SELECT 1 FROM processos_fila
    WHERE responsavel_avaliacao = p_profile_id
      AND status_avaliacao = 'EM_ANALISE'
      AND lote_id = p_lote_id
  ) INTO v_tem_processo_em_analise;

  -- Se já tem processo em análise, não permite capturar outro
  IF v_tem_processo_em_analise THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'JA_TEM_PROCESSO',
      'message', 'Você já possui um processo em análise. Finalize ou salve antes de capturar outro.'
    );
  END IF;
  ...
  -- 3. Verificar se encontrou um processo
